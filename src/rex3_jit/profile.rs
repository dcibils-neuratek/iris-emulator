...
const PROFILE_VERSION: u8 = 2; // v2: triples (dm0, dm1, clipmode_key)
...
pub fn load_profile() -> Vec<(u32, u32, u32)> {
    ...
    for _ in 0..count {
        ...
        let cm  = u32::from_le_bytes(buf[8..12].try_into().unwrap());
...
/// Save (dm0, dm1, clipmode_key) triples to disk.
pub fn save_profile(entries: &[(u32, u32, u32)]) -> io::Result<()> {
    let path = profile_path();
